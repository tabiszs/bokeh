...
namespace mini::gk2 {
    class ModelLoader {
    public:
        explicit ModelLoader(const DxDevice& device) : m_device(device) {}
        ...
        Model LoadFromString(const std::string& modelDescription, InputLayoutManager& layouts,
                             bool smoothNormals = true);

    private:
        static void initLoader(Assimp::Importer& importer);

        static const aiScene* readFromFile(const std::string& filename, Assimp::Importer& importer,
                                           bool smoothNormals);

        static const aiScene* readFromMemory(const std::string& buffer, Assimp::Importer& importer,
                                             bool smoothNormals);
