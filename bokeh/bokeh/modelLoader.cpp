...
static constexpr unsigned int RemoveComponentFlags = aiComponent_ANIMATIONS | aiComponent_BONEWEIGHTS |
                                                     aiComponent_CAMERAS | aiComponent_COLORS | aiComponent_LIGHTS |
...
static constexpr DXGI_FORMAT ComponentFormats[4] = {
...
static constexpr D3D11_INPUT_ELEMENT_DESC PositionElement{
    "POSITION",
    0,
    ComponentFormats[2],
    0,
...
static constexpr D3D11_INPUT_ELEMENT_DESC NormalElement{
    "NORMAL",
    0,
    ComponentFormats[2],
    1,
...
static constexpr D3D11_INPUT_ELEMENT_DESC TexCoordElement(const unsigned int index, const unsigned int components) {
    return {"TEXCOORD", index, ComponentFormats[components - 1], 2 + index, 0, D3D11_INPUT_PER_VERTEX_DATA, 0};
}

static inline unsigned getImportFlags(const bool smoothNormals) {
...
const aiScene* ModelLoader::readFromFile(const string& filename, Importer& importer, const bool smoothNormals) {
    return importer.ReadFile(filename, getImportFlags(smoothNormals));
}

const aiScene* ModelLoader::readFromMemory(const string& buffer, Importer& importer, const bool smoothNormals) {
...
Model ModelLoader::convertToModel(const aiScene* scene, InputLayoutManager& layouts) {
    ...
    for (auto ppMesh = scene->mMeshes; ppMesh < scene->mMeshes + scene->mNumMeshes; ++ppMesh) {
        const auto pAIMesh = *ppMesh;
        assert(
            pAIMesh->HasFaces() && pAIMesh->HasPositions() && pAIMesh->HasNormals() && (pAIMesh->mPrimitiveTypes & ~
                aiPrimitiveType_NGONEncodingFlag) == aiPrimitiveType_TRIANGLE);
        ...
        for (unsigned int texCoordIdx = 0; pAIMesh->HasTextureCoords(texCoordIdx); ++texCoordIdx) {
            const auto texComponenets = pAIMesh->mNumUVComponents[texCoordIdx];
            if (texComponenets == 3) {
                vertexBuffers.push_back(
                    m_device.CreateVertexBuffer(pAIMesh->mTextureCoords[texCoordIdx], pAIMesh->mNumVertices));
            } else {
        ...
        for (unsigned int fIdx = 0; fIdx < pAIMesh->mNumFaces; ++fIdx) {
            const auto& face = pAIMesh->mFaces[fIdx];
...
Model ModelLoader::LoadFromFile(const string& filename, InputLayoutManager& layouts, const bool smoothNormals) {
    Importer importer;
    initLoader(importer);
    return convertToModel(readFromFile(filename, importer, smoothNormals), layouts);
}

Model ModelLoader::LoadFromString(const string& modelDescription, InputLayoutManager& layouts, const bool smoothNormals) {
    Importer importer;
    initLoader(importer);
...
int* ModelLoader::addNode(vector<ModelNode>& nodes, aiNode* pAINode) {
    ...
    const auto myNext = &nodes.back().nextIndex;
    if (pAINode->mNumMeshes == 1)
        nodes.back().meshIndex = pAINode->mMeshes[0];
    auto pChildIndex = &nodes.back().childIndex;
