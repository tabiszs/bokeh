...
void RenderPass::_initShaders(const DxDevice& device, const CBVariableManager& variables,
    ...
    D3D11_SHADER_DESC desc;
    const auto gsRefl = _reflectShader(gsCode, desc);
...
RenderPass::RenderPass(const DxDevice& device, const CBVariableManager& variables, InputLayoutManager* layouts,
                       const wstring& vsShader, const wstring& psShader) : m_layouts(layouts) {
    _initShaders(device, variables, vsShader, psShader);
}

RenderPass::RenderPass(const DxDevice& device, const CBVariableManager& variables, InputLayoutManager* layouts,
                       const wstring& vsShader, const wstring& gsShader, const wstring& psShader) : m_layouts(layouts) {
    _initShaders(device, variables, vsShader, psShader);
}

void RenderPass::_addRenderTarget(const RenderTargetsEffect& renderTarget, const bool clearRenderTarget) {
...
RenderPass::RenderPass(const DxDevice& device, const CBVariableManager& variables, InputLayoutManager* layouts,
                       const RenderTargetsEffect& renderTarget, const bool clearRenderTarget, const wstring& vsShader,
                       const wstring& psShader) : m_layouts(layouts) {
    AddEffect(RenderTargetsEffect{renderTarget.getViewport(), renderTarget.m_buffers.size()});
    _initShaders(device, variables, vsShader, psShader);
    _addRenderTarget(renderTarget, clearRenderTarget);
}

RenderPass::RenderPass(const DxDevice& device, const CBVariableManager& variables, InputLayoutManager* layouts,
                       const RenderTargetsEffect& renderTarget, const bool clearRenderTarget,
                       const wstring& vsShader, const wstring& gsShader, const wstring& psShader) : m_layouts(layouts) {
    AddEffect(RenderTargetsEffect{renderTarget.getViewport(), nullptr, dx_ptr<ID3D11RenderTargetView>(nullptr)});
...
dx_ptr<ID3D11ShaderReflection>
RenderPass::_reflectShader(const vector<BYTE>& shaderCode, D3D11_SHADER_DESC& shaderDesc) {
    ID3D11ShaderReflection* temp;
    auto hr = D3DReflect(shaderCode.data(), shaderCode.size(), __uuidof(ID3D11ShaderReflection),
...
vector<CBufferDesc> RenderPass::_getConstantbuffer_infos(const dx_ptr<ID3D11ShaderReflection>& shaderRefl,
                                                         const D3D11_SHADER_DESC& shaderDesc) {
    vector<CBufferDesc> result;
    result.reserve(shaderDesc.ConstantBuffers);
    for (unsigned int i = 0; i < shaderDesc.ConstantBuffers; ++i) {
        const auto cbuffer = shaderRefl->GetConstantBufferByIndex(i);
        //not COM interface, no need to Release() it
        ...
        for (unsigned int j = 0; j < cbufferDesc.Variables; ++j) {
            const auto cbVar = cbuffer->GetVariableByIndex(j);
            //not COM interface, no need to Release() it
...
vector<string> RenderPass::_getNames(const dx_ptr<ID3D11ShaderReflection>& shaderRefl,
                                     const D3D11_SHADER_DESC& shaderDesc, const D3D_SHADER_INPUT_TYPE type) {
