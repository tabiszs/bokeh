...
namespace mini {
    ...
    namespace gk2 {
        class RenderPass {
            ...
            class CBVariablesEffect final: public ConstantBufferEffectT, public ICBVariablesEffect {
            public:
                using MyBase = ConstantBufferEffectT;
                using MyResourceSetT = ResourceSet<ID3D11Buffer>;

                static_assert(std::is_base_of_v<EffectComponent, ConstantBufferEffectT> &&
                              std::is_base_of_v<MyResourceSetT, ConstantBufferEffectT>,
                              "Base class of CBVariableEffect needst to be a shader constant buffer effect type");


                CBVariablesEffect(const DxDevice& device, std::vector<CBufferDesc>&& buffers) : m_bufferDescriptions(
                    std::move(buffers)) {
                    for (unsigned int i = 0; i < m_bufferDescriptions.size(); ++i) {
                        const auto& [size, variables] = m_bufferDescriptions[i];
                        MyBase::SetResource(i,
                                            device.CreateBuffer(
                                                directx::buffer_info::const_buffer(
                ...
                void Update(const dx_ptr<ID3D11DeviceContext>& context, const CBVariableManager& manager) override {
                    for (unsigned int i = 0; i < m_bufferDescriptions.size(); ++i) {
                        ...
                        const auto hr = context->Map(cbuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &resource);
                        if (FAILED(hr))
        ...
        public:
            ...
            RenderPass(const DxDevice& device, const CBVariableManager& variables, InputLayoutManager* layouts,
                       const RenderTargetsEffect& renderTarget, const std::wstring& vsShader,
                       const std::wstring& psShader) : RenderPass(device, variables, layouts, renderTarget, false,
                                                                  vsShader, psShader) {}

            RenderPass(const DxDevice& device, const CBVariableManager& variables, InputLayoutManager* layouts,
                       const RenderTargetsEffect& renderTarget, const std::wstring& vsShader,
                       const std::wstring& gsShader,
                       const std::wstring& psShader) : RenderPass(device, variables, layouts, renderTarget, false,
                                                                  vsShader, gsShader, psShader) {}
            ...
            std::enable_if_t<std::is_base_of_v<EffectComponent, T>> EmplaceEffect(TArgs&&... args) {
        ...
        private:
            ...
            static dx_ptr<ID3D11ShaderReflection> _reflectShader(const std::vector<BYTE>& shaderCode,
                                                                 D3D11_SHADER_DESC& shaderDesc);

            std::vector<CBufferDesc> _getConstantbuffer_infos(const dx_ptr<ID3D11ShaderReflection>& shaderRefl,
            ...
            template <typename ConstantBufferEffectT>
            void _addShaderConstantBuffers(const DxDevice& device, const dx_ptr<ID3D11ShaderReflection>& shaderRefl,
                                           const D3D11_SHADER_DESC& shaderDesc) {
                auto buffers = _getConstantbuffer_infos(shaderRefl, shaderDesc);
                if (!buffers.empty()) {
            ...
            static std::vector<std::string> _getNames(const dx_ptr<ID3D11ShaderReflection>& shaderRefl,
                                                      const D3D11_SHADER_DESC& shaderDesc, D3D_SHADER_INPUT_TYPE type);

            template <typename SamplersEffectT>
            void _addShaderSamplers(const CBVariableManager& variables,
                                    const dx_ptr<ID3D11ShaderReflection>& shaderRefl,
                                    const D3D11_SHADER_DESC& shaderDesc) {
                const auto samplerNames = _getNames(shaderRefl, shaderDesc, D3D_SIT_SAMPLER);
            ...
            void _addShaderTextures(const CBVariableManager& variables,
                                    const dx_ptr<ID3D11ShaderReflection>& shaderRefl,
                                    const D3D11_SHADER_DESC& shaderDesc) {
                const auto textureNames = _getNames(shaderRefl, shaderDesc, D3D_SIT_TEXTURE);
