...
namespace mini
{
	...
	public:
		...
		explicit ResourceSet(size_t count = 0)
			: m_buffers(count)
		{}
...
#define DEFINE_SHADER_RESOURCE_SET_CLASSES(RESOURCE_TYPE, PTR_TYPE) \
	...
	public:
		...
		RenderTargetsEffect(const RenderTargetsEffect& other)
			: RenderTargetSet(other), m_viewport(other.m_viewport), m_depthBuffer(directx::clone(other.m_depthBuffer)), m_clearOnBegin(other.m_clearOnBegin)
		{ }

		template<typename... ARGS>
		RenderTargetsEffect(const directx::viewport& viewport, dx_ptr<ID3D11DepthStencilView>&& depthBuffer, dx_ptr<ARGS>&&... args)
			: ResourceSet(args...), m_viewport(viewport), m_depthBuffer(std::move(depthBuffer)), m_clearOnBegin(false)
		{ }
		
		RenderTargetsEffect(const directx::viewport &viewport, const size_t target_count = 0U)
			: ResourceSet{ target_count }, m_viewport{ viewport}, m_depthBuffer{ nullptr }, m_clearOnBegin{ false }
		{ }

		template<typename... ARGS>
		explicit RenderTargetsEffect(const directx::viewport viewport, dx_ptr<ARGS>&& ...args)
		...
		void SetViewport(const directx::viewport& viewport)
		{
			m_viewport = viewport;
		}
		...
		const directx::viewport& getViewport() const { return m_viewport; }

	private:
		directx::viewport m_viewport;
